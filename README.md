...
| **供应商管理** | ❌ 无 | ✅ `manage_providers` 注册/移除/列出自定义 OpenAI-compatible 供应商，自动发现模型、上下文窗口、思考支持。流式兼容 `finish-reason-fallback` 模式 |
...
### 5. 模型热切换 + 供应商管理 — `model-switch.ts` + `provider-manager.ts`
...
| 工具/命令 | 能力 |
|------|------|
| `switch_model` 工具 | 列出/切换模型，管理 tier 层级（L0/L1/L2）、思考深度配置。自动应用层级默认思考级别 |
| `manage_providers` 工具 | 注册/移除/列出自定义供应商。支持 openai/anthropic 风格自动检测，`finish-reason-fallback` 容错模式，`supportsUsageInStreaming` 开关。模型从 `/v1/models` 自动发现 |
...
**供应商管理**：`manage_providers` 工具支持注册任意 OpenAI-compatible 或 Anthropic 风格的供应商，自动检测可用模型、上下文窗口、思考支持。`finish-reason-fallback` 模式解决上游流缺少 `finish_reason` 导致的 `Stream ended without finish_reason` 错误。
...
│   ├── model-switch.ts          # 模型热切换 + tier 层级 + 默认持久
│   ├── provider-manager.ts      # ⭐ 自定义供应商管理 + 流式兼容 + tier 层级 + 默认持久
│   ├── provider-manager.ts      # ⭐ 自定义供应商管理 + 流式兼容
