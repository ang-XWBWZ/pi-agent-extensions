...
export function registerManageProviders(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    parameters: Type.Object({
      action: Type.Optional(Type.String({ description: "register|remove|list|set_reasoning_models|set_context_window|set_model_limits" })),
    ...
    async execute(_id, params, _sig, _up, ctx) {
      ...
      if (action !== "register") {
        return { content: [{ type: "text", text: `未知 action: ${action}。支持: register|remove|list|set_reasoning_models|set_context_window|set_model_limits` }], details: {} };
      }
