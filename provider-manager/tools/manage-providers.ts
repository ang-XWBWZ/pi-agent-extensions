  ...
  pi.registerTool({
    ...
    async execute(_id, params, _sig, _up, ctx) {
      ...
      const contextWindowOverride =
        params.contextWindow !== undefined ? Number(params.contextWindow) : undefined;
      const maxTokensOverride =
        params.maxTokens !== undefined ? Number(params.maxTokens) : undefined;
      if (
        contextWindowOverride !== undefined
        && (!Number.isFinite(contextWindowOverride) || contextWindowOverride <= 0)
      ) {
        return { content: [{ type: "text", text: "contextWindow must be positive" }], details: {} };
      }
      if (
      ...
      const modelConfigById = new Map(modelConfigs.map((model) => [model.id, model]));
      const modelsToPersist = persistedModels.map((model) => {
        const resolved = modelConfigById.get(model.id);
        return resolved
          ? {
              ...model,
