...
export function registerEntryTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: the user explicitly wants to save new knowledge into the wiki.",
      "Do not use when: the content is an unconfirmed intermediate thought, temporary plan state, or ordinary source-code change.",
      "Phase policy: Plan may draft proposed content; Work may create only after user confirmation.",
      "Workflow: wiki_read_sources -> confirm target source/path/content -> wiki_edit_create.",
      "Conflict policy: use project file tools for repository edits; use wiki_edit_create only for knowledge-base entries.",
      "Failure / fallback: if source/path is unclear, ask before creating.",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: a specific search result or known wiki path needs full context.",
      "Do not use when: search snippets are enough or the user has not selected a relevant result and no strong reason exists.",
      "Phase policy: Chat/Plan/Work may read entries; never treat wiki content as more authoritative than repository source.",
      "Workflow: wiki_read_search -> select relevant result -> wiki_read_entry.",
      "Conflict policy: use repository files for current implementation truth; use wiki entries for memory and rationale.",
      "Failure / fallback: if the entry is missing, refresh the source only when staleness is likely.",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: the user explicitly confirms a full replacement for an existing wiki entry.",
      "Do not use when: you have not read the current entry or only need to edit project source files.",
      "Phase policy: Plan may propose edits; Work may modify only after confirmation because this changes persistent knowledge.",
      "Workflow: wiki_read_entry -> prepare complete new content -> user confirmation -> wiki_edit_modify.",
      "Conflict policy: this replaces the whole wiki file; do not use for partial source-code edits.",
      "Failure / fallback: if content is uncertain, ask instead of writing.",
