...
export function registerCompileTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use wiki_read_chunks to inspect per-file semantic compilation status before selecting files to compile.",
      "Use wiki_read_chunks pagination or source filters instead of requesting an unbounded status list.",
    ],
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use wiki_DANGER_compile only for selected important files after wiki_read_chunks shows compilation is needed.",
      "After wiki_DANGER_compile, run one bounded compiler and pass its valid JSON to wiki_DANGER_store for the same file.",
      "Use wiki_DANGER_compile force=true only when the existing file lock is confirmed stale.",
    ],
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use wiki_DANGER_store only with valid compiler JSON produced from wiki_DANGER_compile for the same source and relPath.",
      "Never use wiki_DANGER_store with guessed, partial, or manually fabricated semantic data.",
    ],
    parameters: Type.Object({
      source: Type.String({ description: "数据源路径" }),
