...
export function registerCompileTools(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: the user wants higher-quality semantic wiki retrieval for specific important notes.",
      "Do not use when: keyword/hybrid search is sufficient, the file is unrelated, or the user did not authorize indexing work.",
      "Phase policy: Plan may identify candidates; Work may compile only selected files with explicit intent.",
      "Workflow: wiki_read_chunks -> choose one file -> wiki_DANGER_compile -> spawn bounded LLM compiler -> wiki_DANGER_store.",
      "Conflict policy: this improves wiki search metadata; it does not edit source knowledge content.",
      "Failure / fallback: if locked, wait or use force only after confirming the previous compile is stale.",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: wiki_DANGER_compile produced a prompt and a bounded LLM compiler returned valid JSON.",
      "Do not use when: the data is guessed, partial, manually invented, or for a different relPath.",
      "Phase policy: Work only; this writes persistent semantic metadata.",
      "Workflow: validate the LLM output shape -> store with the same source and relPath -> retry search if needed.",
      "Conflict policy: this stores semantic metadata, not wiki article content.",
      "Failure / fallback: if parsing fails, ask the compiler to return only valid JSON once.",
