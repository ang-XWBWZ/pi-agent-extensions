...
export function registerSourceTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: the user explicitly wants to add a knowledge directory or search cannot proceed because no source is loaded.",
      "Do not use when: ordinary repository search/read can answer the question.",
      "Phase policy: Plan may propose loading a source; Work may load only with user intent because this changes persistent wiki state.",
      "Workflow: wiki_read_sources -> wiki_DANGER_load(path) -> wiki_read_search(query).",
      "Conflict policy: this manages wiki indexing, not project files. Do not use it as a replacement for repo search.",
      "Failure / fallback: if the path does not exist, report the resolved path and ask for the correct source.",
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: indexed wiki content is stale after external file changes or semantic vectors must be rebuilt.",
      "Do not use when: wiki tools already updated the index or the user only asked to read/search.",
      "Phase policy: Plan may recommend refresh; Work may refresh only for an explicit stale-index reason.",
      "Workflow: identify stale source -> refresh only that source or file -> retry the search.",
      "Conflict policy: refresh is index maintenance, not knowledge editing.",
      "Failure / fallback: if no sources are loaded, list sources or ask the user to load one.",
