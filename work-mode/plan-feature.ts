...
import { type ConversationPhase, type AppState, type StepStatus, type PlanStep, MAX_PLAN_STEPS, DEFAULT_VISIBLE_STEPS } from "./types.js";
...
export function setupPlanFeature(
    ...
    if (!s.planProduced && s.planSteps.length === 0) {
      ...
      if (hasExplicitPlan) {
        ...
        if (parsed.length > 0) {
          ...
          updatePlanPanel(ctx);
          if (s.phase !== "plan") { cb.setPhase("plan", ctx); }
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      "Use when: Work has started and you need to show or update execution progress.",
      "Do not use when: you are still confirming requirements, scope, assumptions, or acceptance criteria. Use manage_requirements instead.",
      "Phase policy: Plan may inspect status only; Work should use set_steps before multi-step execution and set_step_status as work progresses.",
      "Workflow: set_steps -> set_step_status current/done/error -> insert/update if the execution path changes -> complete when all work and verification are done.",
      "Conflict policy: manage_requirements owns demand confirmation; manage_plan owns implementation progress.",
      "Failure / fallback: if a step fails, mark it error and either re-plan the remaining execution or ask the user about the blocking decision.",
      ...
      "Advance one step at a time via set_step_status. Mark errors honestly; do not skip silently.",
